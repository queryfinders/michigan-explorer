...
<div class="row">
  <div class="col-md-6 mb-3">
    ...
    <input type="text" class="form-control @error('name') is-invalid @enderror" id="name" name="name" value="{{ old('name', $bookingFeature->name ?? '') }}" required />
  ...
  <div class="col-md-6 mb-3">
    ...
    <div class="dropdown">
      <button class="btn btn-outline-secondary w-100 text-start d-flex justify-content-between align-items-center" type="button" id="iconDropdownButton" data-bs-toggle="dropdown" aria-expanded="false" style="background: #fff; border: 1px solid #d9dee3; padding: 8px 12px; height: 38px;">
        <span><i id="selected_icon_display" class="{{ old('icon', $bookingFeature->icon ?? 'fas fa-check') }} me-2"></i> <span id="selected_icon_text">{{ old('icon', $bookingFeature->icon ?? 'fas fa-check') }}</span></span>
    ...
    <input type="hidden" name="icon" id="icon_input" value="{{ old('icon', $bookingFeature->icon ?? 'fas fa-check') }}">
    @error('icon')
  ...
  <div class="col-md-6 mb-3">
    ...
    <input type="number" class="form-control @error('sort_order') is-invalid @enderror" id="sort_order" name="sort_order" value="{{ old('sort_order', $bookingFeature->sort_order ?? 0) }}" required />
    @error('sort_order')<div class="invalid-feedback">{{ $message }}</div>@enderror
  </div>

  <input type="hidden" name="is_active" value="{{ old('is_active', $bookingFeature->is_active ?? 1) }}" />
</div>

<div class="mt-4">
  <button type="submit" class="btn btn-warning me-2">{{ isset($bookingFeature) ? 'Update Changes' : 'Save Changes' }}</button>
