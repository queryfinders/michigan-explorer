...
<div class="mb-3">
  ...
  <textarea class="form-control tinymce-editor" id="description" name="description">{{ old('description', $restaurantCategory->description ?? '') }}</textarea>
</div>
<input type="hidden" name="status" value="{{ old('status', $restaurantCategory->status ?? 1) }}">
<button type="submit" class="btn btn-warning">{{ isset($restaurantCategory) ? 'Update' : 'Save' }}</button>
