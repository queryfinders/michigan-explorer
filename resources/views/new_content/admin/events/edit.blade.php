      ...
      <div class="tab-content p-0" id="eventFormTabsContent">
        ...
        <div class="tab-pane fade show active" id="basic-pane" role="tabpanel" aria-labelledby="basic-tab">
          <div class="row">
            <div class="col-md-4 mb-3">
              ...
              <input type="hidden" name="event_category_id" id="event_category_id" value="{{ old('event_category_id', $event->event_category_id) }}" required />
                  ...
                  <div class="cuisine-items-list" id="categoryItemsList">
                    ...
                    <label class="cuisine-item" id="cat-label-{{ $cat->id }}">
                      <input type="radio" name="_cat_radio" value="{{ $cat->id }}"
                             ...
                             data-name="{{ $cat->name }}"
                             data-id="{{ $cat->id }}"
                             {{ old('event_category_id', $event->event_category_id) == $cat->id ? 'checked' : '' }}
            ...
            <div class="col-md-4 mb-3">
              ...
              <input type="text" class="form-control" id="name" name="name" value="{{ old('name', $event->name) }}" required placeholder="e.g. Grand Rapids Art Festival" />
            </div>
            <div class="col-md-4 mb-3">
              <label class="form-label fw-semibold" for="slug">Slug <span class="text-danger">*</span></label>
              <input type="text" class="form-control" id="slug" name="slug" value="{{ old('slug', $event->slug) }}" required placeholder="e.g. grand-rapids-art-festival" />
            </div>
          </div>
          ...
          <div class="row">
            <div class="col-md-6 mb-3">
              ...
              <input type="datetime-local" class="form-control" id="start_date" name="start_date" value="{{ old('start_date', $event->start_date ? date('Y-m-d\TH:i', strtotime($event->start_date)) : '') }}" />
            </div>
            <div class="col-md-6 mb-3">
              <label class="form-label fw-semibold" for="end_date">End Date & Time</label>
              <input type="datetime-local" class="form-control" id="end_date" name="end_date" value="{{ old('end_date', $event->end_date ? date('Y-m-d\TH:i', strtotime($event->end_date)) : '') }}" />
            </div>
          </div>
          ...
          <div class="row">
            <div class="col-md-4 mb-3">
              ...
              <input type="text" class="form-control" id="venue_name" name="venue_name" value="{{ old('venue_name', $event->venue_name) }}" placeholder="e.g. Calder Plaza" />
            </div>
            <div class="col-md-4 mb-3">
              <label class="form-label fw-semibold" for="city">City</label>
              <input type="hidden" name="city" id="city_hidden" value="{{ old('city', $event->city) }}" />
                  ...
                  <div class="cuisine-items-list" id="cityItemsList">
                    @foreach(config('michigan_cities') as $m_city)
                      ...
                      <label class="cuisine-item" id="city-label-{{ $m_city_slug }}">
                        <input type="radio" name="_city_radio" value="{{ $m_city }}" id="city_rb_{{ $m_city_slug }}" class="city-rb d-none" data-name="{{ $m_city }}" {{ old('city', $event->city) == $m_city ? 'checked' : '' }} onchange="onCityChange(this)" />
          ...
          <div class="row">
            <div class="col-md-12 mb-3">
              ...
              <textarea class="form-control" id="map_iframe" name="map_iframe" rows="3" placeholder="Paste Google Maps iframe code here">{{ old('map_iframe', $event->map_iframe ?? '') }}</textarea>
            </div>
          </div>

          <div class="mb-3">
            <label class="form-label fw-semibold" for="description">Description</label>
            <textarea class="form-control tinymce" id="description" name="description" rows="6">{{ old('description', $event->description) }}</textarea>
          </div>
          <div class="mb-3">
            <div class="form-check form-switch mt-2">
              <input class="form-check-input" type="checkbox" id="is_featured" name="is_featured" value="1" {{ old('is_featured', $event->is_featured ?? 0) ? 'checked' : '' }}>
        ...
        <div class="tab-pane fade" id="featured-pane" role="tabpanel" aria-labelledby="featured-tab">
          ...
          <div class="mb-3 border-top pt-3">
            ...
            <input type="url" class="form-control" id="video_url" name="video_url" value="{{ old('video_url', str_starts_with($event->video ?? '', 'http') ? $event->video : '') }}" placeholder="e.g. https://www.youtube.com/watch?v=dQw4w9WgXcQ" />
        ...
        <div class="tab-pane fade" id="seo-pane" role="tabpanel" aria-labelledby="seo-tab">
          <div class="mb-3">
            ...
            <input type="text" class="form-control" id="meta_title" name="meta_title" value="{{ old('meta_title', $event->seo->meta_title ?? '') }}" placeholder="e.g. Best Festival in GR | Michigan Explorer" />
          </div>
          <div class="mb-3">
            <label class="form-label fw-semibold" for="meta_description">Meta Description</label>
            <textarea class="form-control" id="meta_description" name="meta_description" rows="2" placeholder="e.g. Discover the best festival...">{{ old('meta_description', $event->seo->meta_description ?? '') }}</textarea>
          </div>
          <div class="mb-3">
            <label class="form-label fw-semibold" for="og_title">OG Title</label>
            <input type="text" class="form-control" id="og_title" name="og_title" value="{{ old('og_title', $event->seo->og_title ?? '') }}" />
          </div>
          <div class="mb-3">
            <label class="form-label fw-semibold" for="og_description">OG Description</label>
            <textarea class="form-control" id="og_description" name="og_description" rows="2">{{ old('og_description', $event->seo->og_description ?? '') }}</textarea>
          </div>
          <div class="mb-3">
            <label class="form-label" for="schema_markup">Schema Markup (JSON-LD) - <span class="text-info">Auto-generated</span></label>
            <textarea class="form-control" id="schema_markup" name="schema_markup" rows="8" placeholder="Auto-generated on save" readonly disabled>{{ $event->seo->schema_markup ?? '' }}</textarea>
          </div>
        </div>
      </div>
...
<script>
  ...
  function previewFeaturedImage(event) {
    ...
    reader.onload = e => {
      ...
      const currentImg = document.getElementById('current-featured-img');
      if (currentImg) currentImg.style.display = 'none';
    };
    reader.readAsDataURL(file);
  }

  let faqIndex = {{ $event->faqs ? count($event->faqs) : 0 }};
  function addFaq() {
