...
<div class="collapse mb-4 {{ request()->anyFilled(['search', 'status']) ? 'show' : '' }}" id="filterCollapse">
  <div class="card card-body">
    <form id="filterForm" method="GET" action="{{ route('event-categories.index') }}">
      <div class="row g-3">
        <div class="col-md-5">
        ...
        <div class="col-md-3 d-flex align-items-end">
          <div class="d-flex gap-2 w-100">
            <a href="{{ route('event-categories.index') }}" class="btn btn-secondary w-50">Reset</a>
...
<div class="card">
  <div id="ajax-table-container">
    @include('new_content.admin.event_categories._table')
  </div>
</div>
@endsection
...
<script>
  $(document).ready(function() {
      $(document).on('change', '.category-status-switch', function (e) {
          ...
          $.ajax({
              url: '{{ url("admin/event-categories/status") }}/' + id + '/' + status,
              type: 'GET',
