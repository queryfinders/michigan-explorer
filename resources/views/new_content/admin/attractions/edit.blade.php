                  ...
                  <div class="cuisine-items-list" id="categoryItemsList">
                    ...
                    <label class="cuisine-item" id="cat-label-{{ $cat->id }}">
                      <input type="radio" name="_cat_radio" value="{{ $cat->id }}"
                             ...
                             data-name="{{ $cat->name }}"
                             data-id="{{ $cat->id }}"
                             {{ old('attraction_category_id') == $cat->id ? 'checked' : '' }}
...
// Icon Picker Search (Amenity)
  ...
  function saveNewAttractionPolicy() {
    ...
    $.ajax({
      ...
      success: function(response) {
        if (response.success) {
          const p = response.attraction_policy;
          const container = document.querySelector('label:contains("Attraction Policies")').closest('.row.g-3') || document.querySelector('.row.g-3').lastElementChild.parentElement;
          const div = document.createElement('div');
          div.className = 'col-md-6';
          let inputHtml = type === 'textarea' ? '<textarea class="form-control" name="attraction_policies['+p.id+']" id="policy_'+p.id+'" rows="2"></textarea>' : '<input type="text" class="form-control" name="attraction_policies['+p.id+']" id="policy_'+p.id+'" />';
          ...
          bootstrap.Modal.getInstance(document.getElementById('addAttractionPolicyModal')).hide();
        } else { alertBox.className = 'alert alert-danger'; alertBox.textContent = response.message || 'Failed.'; }
      },
      error: function(xhr) {
        alertBox.className = 'alert alert-danger';
