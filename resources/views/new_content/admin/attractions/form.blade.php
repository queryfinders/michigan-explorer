<!-- Tabs Navigation -->
<ul class="nav nav-tabs mb-4" id="attractionFormTabs" role="tablist">
...
<!-- Tabs Content -->
<div class="tab-content p-0" id="attractionFormTabsContent">
  ...
  <div class="tab-pane fade show active" id="basic-pane" role="tabpanel" aria-labelledby="basic-tab">
            ...
            <div class="cuisine-items-list" id="categoryItemsList">
              ...
              <label class="cuisine-item" id="cat-label-{{ $cat->id }}">
                <input type="radio" name="_cat_radio" value="{{ $cat->id }}"
                       ...
                       data-name="{{ $cat->name }}"
                       data-id="{{ $cat->id }}"
                       {{ old('attraction_category_id', isset($attraction) ? $attraction->attraction_category_id : '') == $cat->id ? 'checked' : '' }}
    ...
    <div class="mb-3">
      ...
      <textarea class="form-control tinymce" id="description" name="description" rows="6">{{ old('description', isset($attraction) ? $attraction->description : '') }}</textarea>
    </div>
    <div class="mb-3">
      <div class="form-check form-switch mt-2">
        <input class="form-check-input" type="checkbox" id="is_featured" name="is_featured" value="1" {{ old('is_featured', isset($attraction) ? $attraction->is_featured : 0) ? 'checked' : '' }}>
  ...
  <div class="tab-pane fade" id="details-pane" role="tabpanel" aria-labelledby="details-tab">
    <div class="row">
      <div class="col-md-4 mb-3">
        ...
        <input type="hidden" name="city" id="city_hidden" value="{{ old('city', isset($attraction) ? $attraction->city : '') }}" />
      ...
      <div class="col-md-4 mb-3">
        ...
        <input type="text" class="form-control" id="zip" name="zip" placeholder="e.g. 49757" value="{{ old('zip', isset($attraction) ? $attraction->zip : '') }}" />
      </div>
      <div class="col-md-4 mb-3">
        <label class="form-label" for="address">Street Address</label>
        <input type="text" class="form-control" id="address" name="address" placeholder="e.g. 286 Grand Avenue" value="{{ old('address', isset($attraction) ? $attraction->address : '') }}" />
      </div>
    </div>
    <div class="row">
      <div class="col-md-4 mb-3">
        <label class="form-label" for="phone">Phone Number</label>
        <input type="text" class="form-control" id="phone" name="phone" placeholder="e.g. +1 555-123-4567" value="{{ old('phone', isset($attraction) ? $attraction->phone : '') }}" />
      </div>
      <div class="col-md-4 mb-3">
        <label class="form-label" for="email">Email</label>
        <input type="email" class="form-control" id="email" name="email" placeholder="e.g. info@example.com" value="{{ old('email', isset($attraction) ? $attraction->email : '') }}" />
      </div>
      <div class="col-md-4 mb-3">
        <label class="form-label" for="website">Website URL</label>
        <input type="url" class="form-control" id="website" name="website" placeholder="e.g. https://www.example.com" value="{{ old('website', isset($attraction) ? $attraction->website : '') }}" />
      </div>
    </div>
    <div class="row">
      <div class="col-md-12 mb-3">
        <label class="form-label" for="map_iframe">Map Iframe Code</label>
        <textarea class="form-control" id="map_iframe" name="map_iframe" rows="3" placeholder="Paste Google Maps iframe code here">{{ old('map_iframe', isset($attraction) ? $attraction->map_iframe : '') }}</textarea>
      </div>
    </div>
  </div>
  ...
  <div class="tab-pane fade" id="featured-pane" role="tabpanel" aria-labelledby="featured-tab">
    ...
    @if(isset($attraction) && $attraction->featured_image)
      <div class="mb-3" id="featured-preview-existing">
        <label class="form-label text-muted small">Current Featured Image</label>
        <div>
          <img src="{{ asset($attraction->featured_image) }}" alt="{{ $attraction->featured_image_alt ?? 'Featured Image' }}" class="img-thumbnail" style="max-height:220px;" />
        </div>
      </div>
    @endif
    ...
    <div class="mb-3">
      ...
      <input type="text" class="form-control" id="featured_image_alt" name="featured_image_alt" value="{{ old('featured_image_alt', isset($attraction) ? $attraction->featured_image_alt : '') }}" />
  ...
  <div class="tab-pane fade" id="gallery-pane" role="tabpanel" aria-labelledby="gallery-tab">
    ...
    <div class="mb-4">
      ...
      <div class="row g-3" id="existing-gallery-grid">
        @if(!empty($attraction->video) && !str_starts_with($attraction->video, 'http'))
  ...
  <div class="tab-pane fade" id="seo-pane" role="tabpanel" aria-labelledby="seo-tab">
    <div class="mb-3">
      ...
      <input type="text" class="form-control" id="meta_title" name="meta_title" placeholder="e.g. Best Attraction in Michigan | Michigan Explorer" value="{{ old('meta_title', isset($attraction) && $attraction->seo ? $attraction->seo->meta_title : '') }}" />
    </div>
    <div class="mb-3">
      ...
      <textarea class="form-control" id="meta_description" name="meta_description" placeholder="e.g. Discover the best attraction in Michigan..." maxlength="160" rows="2">{{ old('meta_description', isset($attraction) && $attraction->seo ? $attraction->seo->meta_description : '') }}</textarea>
    </div>
    <div class="mb-3">
        <label class="form-label" for="og_title">OG Title</label>
        <input type="text" class="form-control" id="og_title" name="og_title" placeholder="e.g. Best Attraction in Michigan" value="{{ old('og_title', isset($attraction) && $attraction->seo ? $attraction->seo->og_title : '') }}" />
      </div>
    <div class="mb-3">
      ...
      <textarea class="form-control" id="og_description" name="og_description" placeholder="e.g. Discover the best attraction in Michigan..." maxlength="160" rows="2">{{ old('og_description', isset($attraction) && $attraction->seo ? $attraction->seo->og_description : '') }}</textarea>
    </div>
    <div class="mb-3">
      <label class="form-label" for="schema_markup">Schema Markup (JSON-LD) - <span class="text-info">Auto-generated</span></label>
      <textarea class="form-control" id="schema_markup" name="schema_markup" rows="8" placeholder="Auto-generated on save" readonly disabled>{{ old('schema_markup', isset($attraction) && $attraction->seo ? $attraction->seo->schema_markup : '') }}</textarea>
    </div>
  </div>
</div>
...
<script>
  ...
  let faqIndex = {{ isset($attraction) ? 'Date.now()' : '0' }};
  function addFaq() {
...
<script>
  function saveNewCategory() {
    ...
    $.ajax({
      url: '{{ route("attraction-categories.quick-store") }}', type: 'POST',
  ...
  function onCategoryChange(rb) {
    ...
    const hidden= document.getElementById('attraction_category_id');
    ...
    // Attractions uses standard error label
  }
  ...
  document.addEventListener('DOMContentLoaded', function() {
    const preselectedCat = document.getElementById('attraction_category_id').value;
    if (preselectedCat) {
