...
<div class="row mb-3">
  <div class="col-md-6">
    ...
    <input type="text" class="form-control @error('name') is-invalid @enderror" id="name" name="name" placeholder="e.g. Free WiFi" value="{{ old('name', $amenity->name ?? '') }}" required />
    @error('name')
  ...
  <div class="col-md-6">
    ...
    <input type="text" class="form-control @error('slug') is-invalid @enderror" id="slug" name="slug" placeholder="e.g. free-wifi" value="{{ old('slug', $amenity->slug ?? '') }}" required />
    @error('slug')
      <div class="invalid-feedback">{{ $message }}</div>
    @enderror
  </div>
...
<div class="mb-3">
  ...
  <div class="dropdown">
    <button class="btn btn-outline-secondary w-100 text-start d-flex justify-content-between align-items-center" type="button" id="iconDropdownButton" data-bs-toggle="dropdown" aria-expanded="false" style="background: #fff; border: 1px solid #d9dee3; padding: 8px 12px; height: 38px;">
      <span><i id="selected_icon_display" class="{{ old('icon', $amenity->icon ?? '') }} me-2"></i> <span id="selected_icon_text">{{ old('icon', $amenity->icon ?? 'Select Icon') }}</span></span>
  ...
  <input type="hidden" name="icon" id="icon_input" value="{{ old('icon', $amenity->icon ?? '') }}">
  @error('icon')
...
<input type="hidden" name="status" value="{{ old('status', $amenity->status ?? 1) }}" />
<button type="submit" class="btn btn-warning">{{ isset($amenity) ? 'Update' : 'Save' }}</button>
