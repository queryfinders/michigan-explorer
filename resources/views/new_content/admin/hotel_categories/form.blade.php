...
<div class="mb-3">
  ...
  <textarea class="form-control tinymce" id="description" name="description">{{ old('description', $hotelCategory->description ?? '') }}</textarea>
</div>
<input type="hidden" name="status" value="{{ old('status', $hotelCategory->status ?? 1) }}" />
<button type="submit" class="btn btn-warning">{{ isset($hotelCategory) ? 'Update' : 'Save' }}</button>
